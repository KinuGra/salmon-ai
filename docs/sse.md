...
↓ PythonにリクエストしてSSEレスポンスを受け取る
↓ Send で1トークンずつフロントに流す
フロントエンド
→ リアルタイムで表示
...
```go
import "github.com/salmon-ai/salmon-ai/pkg/sse"

func (h *AIHandler) ReflectionStream(c *gin.Context) {
    // 必ずSetHeadersを最初に呼ぶ
    sse.SetHeaders(c)

    // Geminiから受け取ったテキストを1トークンずつ流す
...
func (h *AIHandler) ReflectionStream(c *gin.Context) {
    ...
    data, err := h.aiClient.Post("/reflection/stream", req)
    if err != nil {
