...
function EditTaskModal({
  ...
  const [selectedMins, setSelectedMins] = useState<number | null>(initMins);
  const [customH, setCustomH] = useState<string>("");
  const [customM, setCustomM] = useState<string>("0");

  function handleQuickDuration(mins: number) { setSelectedMins(mins); setCustomH(""); setCustomM("0"); }
  function handleCustomDuration(h: string, m: string) { setCustomH(h); setCustomM(m); setSelectedMins(null); }
  const customDurationMins = customH !== "" || customM !== "0" ? (parseInt(customH || "0") * 60) + parseInt(customM) : null;
  const durationMins = selectedMins ?? customDurationMins;

  const initDue = task.due_date ? new Date(task.due_date) : null;
  const [dueDate, setDueDate] = useState<Date | null>(initDue);
  const [customYear,  setCustomYear]  = useState<string>(String(initDue?.getFullYear() ?? THIS_YEAR));
  const [customMonth, setCustomMonth] = useState<string>(initDue ? String(initDue.getMonth() + 1) : "");
  const [customDay,   setCustomDay]   = useState<string>(initDue ? String(initDue.getDate()) : "");
  ...
  function clearDue() { setDueDate(null); setCustomYear(String(THIS_YEAR)); setCustomMonth(""); setCustomDay(""); }
      ...
      <div className="relative bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl p-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            {durationMins !== null && (
              <div className="flex items-baseline justify-center gap-1 bg-indigo-50 rounded-xl py-2 mb-2">
                <span className="text-[28px] font-bold text-indigo-600 leading-none tabular-nums">{fmtMins(durationMins)}</span>
              </div>
            )}
            <div className="flex flex-wrap gap-1.5 mb-2">
              {DURATION_QUICK.map(({ label, mins }) => (
                <button key={mins} type="button" onClick={() => handleQuickDuration(mins)}
                  className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border transition-all active:scale-95 ${selectedMins === mins ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
                  {label}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-2">
              <input type="number" min="0" max="23" value={customH} onChange={(e) => handleCustomDuration(e.target.value, customM)} placeholder="0"
                className="w-16 border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] text-center focus:outline-none focus:ring-2 focus:ring-indigo-300 transition" />
              <span className="text-[13px] text-slate-500 shrink-0">h</span>
              <select value={customM} onChange={(e) => handleCustomDuration(customH, e.target.value)} className="flex-1 border border-slate-200 rounded-xl px-3 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white transition">
                <option value="0">00</option><option value="15">15</option><option value="30">30</option><option value="45">45</option>
              </select>
              <span className="text-[13px] text-slate-500 shrink-0">m</span>
            </div>
          </div>
          <div>
            <label className={labelCls}>期限（任意）</label>
            {dueDate && (
              <div className="flex items-baseline justify-center gap-1.5 bg-indigo-50 rounded-xl py-2 mb-2">
                <span className="text-[28px] font-bold text-indigo-600 leading-none tabular-nums">{fmtMD(dueDate)}</span>
                <span className="text-[13px] text-indigo-400 font-semibold leading-none pb-0.5">{dueDate.getFullYear()}年</span>
              </div>
            )}
            <div className="flex gap-1.5 mb-2">
              {DUE_QUICK.map(({ label, date }) => (
                <button key={label} type="button" onClick={() => handleQuickDue(date)}
                  className={`flex-1 text-[11px] font-bold py-1.5 rounded-lg border transition-all active:scale-95 ${dueDate?.toDateString() === date.toDateString() ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}>
                  {label}
                </button>
              ))}
            </div>
            <div className="flex items-center gap-1.5">
              <select value={customYear} onChange={(e) => handleCustomDue(e.target.value, customMonth, customDay)} className="w-24 border border-slate-200 rounded-xl px-2 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white transition">
                <option value={THIS_YEAR}>{THIS_YEAR}年</option>
                <option value={THIS_YEAR + 1}>{THIS_YEAR + 1}年</option>
              </select>
              <select value={customMonth} onChange={(e) => handleCustomDue(customYear, e.target.value, customDay)} className="flex-1 border border-slate-200 rounded-xl px-2 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white transition">
                <option value="">月</option>
                {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (<option key={m} value={m}>{m}月</option>))}
              </select>
              <select value={customDay} onChange={(e) => handleCustomDue(customYear, customMonth, e.target.value)} className="flex-1 border border-slate-200 rounded-xl px-2 py-2.5 text-[13px] text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white transition">
                <option value="">日</option>
                {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => (<option key={d} value={d}>{d}日</option>))}
              </select>
              {dueDate && <button type="button" onClick={clearDue} className="shrink-0 text-[11px] text-slate-400 hover:text-slate-600 px-1.5">✕</button>}
            </div>
          </div>
        </div>
        <div className="flex gap-2.5 mt-6">
